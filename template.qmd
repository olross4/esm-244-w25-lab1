...
There are 197,618 observations of 12 attributes about urban trees, their location, and management status. Some questions you may ask: 1) Is there a correlation between legal status and site order? 2) Which areas contain the most trees? 3) Which areas have the highest biodiversity of trees?

Question we are answering in lab: How many counts of each observation by legal_status? Focus on the highest trees?

```{r}
top5_sites <- sfo_trees_df |>
...
```{r}
fig <- ggplot(top5_sites, aes(x=fct_reorder(legal_status, tree_count), y=tree_count)) +
...
This code filters the original data frame to include trees that are on  permitted sites that also have caretakers that are MTA and DPW.
...
```{r}
Use %in% when selecting rows from a list of variables. 
...
Remove colums that we do not need for our analysis, filter out plants that are not oak and pine trees, summarize the oak and pine trees 
...
```{r}
ggplot(oak_pine_df, aes(longitude, latitude, color=type))+
  geom_point() +
